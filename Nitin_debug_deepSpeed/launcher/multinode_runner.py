...
class OpenMPIRunner(MultiNodeRunner):
    ...
    def get_cmd(self, environment, active_resources):
        ...
        mpirun_cmd = [
            'mpirun',
            #TODO:Remove after experiment-Nitin
            '--allow-run-as-root',
            '-n',
...
class IMPIRunner(MultiNodeRunner):
    ...
    def get_cmd(self, environment, active_resources):
        ...
        #Case 1: Original IMPI launcher
        #print("nkbhat_dbg: IMPI calling get_cmd from multinode_runner")
        #mpirun_cmd = [
        #    'mpirun',
        #    '-ppn',
        #    f'{process_per_node}',
        ...
        #Case 2: WIP, use OpenMPI's launcher in IMPI style
        print("nkbhat_dbg: IMPI calling get_cmd from multinode_runner, using OpenMPI launcher")
        mpirun_cmd = [
            '/root/nkbhat_debug/openmpi-5.0.5/build/install/bin/mpiexec',
            '-N',
            f'{process_per_node}',
            '--allow-run-as-root',
            #'--host',
            #'JF5300-B11A346T:2,JF5300-B11A319T:2',
        ] + split(self.args.launcher_args)
        export_cmd = []

        for k, v in self.exports.items():
            export_cmd += ['-x', f'{k}='+f'{v}']

        if self.args.bind_cores_to_rank:
            cores_per_rank, _ = get_numactl_cmd(self.args.bind_core_list, process_per_node, 0)
            print("nkbhat_dbg: IMPI calling get_cmd from multinode_runner, using OpenMPI launcher, cores_per_rank=" + str(cores_per_rank))
            export_cmd += ['-x', 'OMP_NUM_THREADS='+str(cores_per_rank)]

        export_cmd += ['-x', 'MASTER_ADDR='+str(self.args.master_addr)]
        export_cmd += ['-x', 'MASTER_PORT='+str(self.args.master_port)]
        export_cmd += ['-x', 'WORLD_SIZE='+str(total_process_count)]
        export_cmd += ['-x', 'LOCAL_SIZE='+str(process_per_node)]
        export_cmd += ['-x', 'I_MPI_PMI=pmix']
        export_cmd += ['-x', 'I_MPI_PMI_LIBRARY=/root/nkbhat_debug/openmpi-5.0.5/build/install/lib/libpmix.so.2.13.3']
        export_cmd += ['-x', 'FI_PROVIDER=tcp']

        # turn off IMPI core binding, use deepspeed's own core binding
        #export_cmd += ['-x', 'I_MPI_PIN=0']
        ...
        for i in range(total_process_count):
            ...
            #if self.args.bind_cores_to_rank:
            #    _, numactl_cmd = get_numactl_cmd(self.args.bind_core_list, process_per_node, local_rank)
            #    print("nkbhat_dbg: IMPI calling get_cmd from multinode_runner, using OpenMPI launcher, numactl_cmd is=" + str(numactl_cmd))
            #    numactl_cmd.append("-a")
            #    python_exec += numactl_cmd
            ...
            env_mapping = ['-x', 'RANK='+str(i)]
            env_mapping += ['-x', 'LOCAL_RANK='+str(local_rank)]
            if i == 0:
                per_host_cmd = ['-n', '1'] + env_mapping + python_exec + [self.user_script] + self.user_arguments
            else:
                per_host_cmd = per_host_cmd + [':', '-n', '1'] + env_mapping + python_exec + [self.user_script

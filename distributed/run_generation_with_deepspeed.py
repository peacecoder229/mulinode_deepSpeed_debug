...
prompt = json.load(open(str(current_path) + "/prompt.json"))[model_type][args.input_tokens]
...
# benchmark it!
    ...
    if args.profile:
        ...
        # Wait for all ranks to finish before move on
        deepspeed.comm.barrier()
    actual_cyc = cycles - warmup
    print_rank0("inftime=%s  BS=%s token=%s\n" %(tot_inf_time / actual_cyc, args.batch_size, args.max_new_tokens))
    print_rank0("gflops=%s\n" %(flops / (10**9)))
    latency = tot_inf_time / (cycles - warmup) * 1000
    print_rank0("\n", "-" * 10, "Summary:", "-" * 10)
    ...
    if args.token_latency:
        ...
        print_rank0("firstlat=%.4f seclat=%.4f\n" % (first_latency, average_2n_latency))
